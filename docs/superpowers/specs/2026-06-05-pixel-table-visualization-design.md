...
**像素风（PixiJS + Sprite）**，调性参考 v3 mockup 的 "明亮卡通 + Cyber 角色"：
- **环境**：暖黄背景渐变 + 草绿桌面 + 木色桌沿（暂不做赛博地砖等环境装饰，v2 候选）
- **角色**：6 个赛博风 AI 牌手（机器人 / 霓虹发色 / AR 眼镜 / 机械臂等 gimmick），每个角色一个独特视觉锚点
- **UI**：圆角白底气泡（非像素风边框），保持现有 Apple 风 UI 的克制感

> **风格冲突的取舍**：UI 走现代 iOS 风（圆角、模糊、Apple 风），**只在牌桌主区是像素风**。两者通过"像素 sprite 渲染在 Canvas 中"的边界自然隔离。这避免整站像素化造成的阅读疲劳。
