...
4. 用户既可使用平台执行的版本化 `Strategy Package`，也可使用经认证的远程 WebSocket/webhook Agent 参赛。
...
经身份认证的 WebSocket 或 webhook Agent 同样可以进入排名赛。它可以自己管理策略程序、LLM 调用、训练成果与运行环境。
...
当前创建流程优先生成声明式策略包，是为了低门槛可玩，不表示用户必须永远使用该 DSL。
