...
export function TabBar({ activeTab, onTabChange, hasNewHighlight }: TabBarProps) {
  return (
    <div className="flex border-b border-gray-800 bg-gray-950/80 backdrop-blur-sm sticky top-0 z-10">
      {TABS.map((tab) => (
        <button
          key={tab.id}
        ...
        >
          ...
          {tab.id === "highlights" && hasNewHighlight && activeTab !== "highlights" && (
            <span className="absolute top-1.5 right-1/4 w-2 h-2 bg-fuchsia-500 rounded-full animate-pulse" />
          )}
        </button>
      ))}
    </div>
